...
def _rates(**kw):
    base = {
        "status": "up",
        "speed_bps": 1000,
        "current_in_bps": 100.0,
        "current_out_bps": 50.0,
...
def test_requires_token_when_configured(client, db_session, monkeypatch):
    ...
    assert client.get("/metrics/prometheus", headers={"Authorization": "Bearer wrong"}).status_code == 401


def test_disabled_when_token_empty(client, monkeypatch):
    monkeypatch.setattr(config.settings, "prometheus_scrape_token", "")
    assert client.get("/metrics/prometheus", headers={"Authorization": "Bearer anything"}).status_code == 503
