...
def device(db_session):
    d = Device(
        name="sw", ip_address="10.0.0.1", snmp_version="2c", snmp_community="public", assigned_agent_id="agent-1"
    )
    db_session.add(d)
    db_session.commit()
...
def test_walk_enqueues_on_assigned_agent(client, admin_headers, device, respx_mock):
    ...
    resp = client.post(f"/devices/{device.id}/walk", params={"base_oid": "1.3.6.1.2.1"}, headers=admin_headers)
...
def test_walk_falls_back_to_online_agent(client, admin_headers, db_session, respx_mock):
    ...
    db_session.commit()
    db_session.refresh(d)
    respx_mock.get("http://manager:8000/agents").mock(
        return_value=httpx.Response(200, json=[{"agent_id": "agent-x", "status": "online"}])
    )
...
def test_get_walk_result_proxied(client, admin_headers, respx_mock):
    respx_mock.get("http://manager:8000/commands/cmd-123").mock(
        return_value=httpx.Response(
            200,
            json={
                "command_id": "cmd-123",
                "status": "done",
                "result": [{"oid": "1.3.6.1.2.1.1.1.0", "value": "sysDescr"}],
                "error": None,
            },
        )
    )
