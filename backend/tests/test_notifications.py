...
def test_create_channel_invalid_type_rejected(client, admin_headers):
    r = client.post(
        "/notification-channels", json={"name": "x", "type": "pagerduty", "url": "http://x"}, headers=admin_headers
    )
...
def test_update_channel(client, admin_headers):
    cid = client.post(
        "/notification-channels", json={"name": "ops", "type": "webhook", "url": "http://x"}, headers=admin_headers
    ).json()["id"]
    r = client.put(
        f"/notification-channels/{cid}", json={"enabled": False, "severity_filter": ["critical"]}, headers=admin_headers
    )
    assert r.status_code == 200
...
def _alert(db, alert_type, severity, message="m", device_id=None):
    a = Alert(alert_type=alert_type, severity=severity, message=message, status=AlertStatus.open, device_id=device_id)
    db.add(a)
    db.commit()
    db.refresh(a)
    return a
...
def test_dispatch_filters_by_severity(db_session, monkeypatch):
    db_session.add(
        NotificationChannel(name="crit-only", type="slack", url="http://s", severity_filter=["critical"], enabled=True)
    )
    db_session.commit()
    sent = []
...
def test_dispatch_empty_filter_matches_all(db_session, monkeypatch):
    db_session.add(NotificationChannel(name="all", type="webhook", url="http://w", severity_filter=[], enabled=True))
    db_session.commit()
    sent = []
    monkeypatch.setattr(notif_service.httpx, "post", lambda url, **k: sent.append(url))
...
def test_dispatch_disabled_channel_skipped(db_session, monkeypatch):
    db_session.add(NotificationChannel(name="off", type="webhook", url="http://w", severity_filter=[], enabled=False))
    db_session.commit()
    sent = []
    monkeypatch.setattr(notif_service.httpx, "post", lambda url, **k: sent.append(url))
...
def test_dispatch_swallows_channel_errors(db_session, monkeypatch):
    db_session.add(NotificationChannel(name="bad", type="webhook", url="http://w", severity_filter=[], enabled=True))
    db_session.commit()
...
def test_dispatch_payload_shapes(db_session, monkeypatch):
    db_session.add(NotificationChannel(name="s", type="slack", url="http://slack", severity_filter=[], enabled=True))
    db_session.add(NotificationChannel(name="w", type="webhook", url="http://wh", severity_filter=[], enabled=True))
    db_session.commit()
    calls = {}
    monkeypatch.setattr(notif_service.httpx, "post", lambda url, json=None, **k: calls.__setitem__(url, json))
    notif_service.dispatch_alert(
        db_session, _alert(db_session, AlertType.interface_down, AlertSeverity.warning, "eth0 down", device_id=5)
    )
