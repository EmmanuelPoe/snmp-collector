...
def upgrade() -> None:
    ...
    op.add_column("alerts", sa.Column("severity", severity, nullable=False, server_default="warning"))
    op.execute("UPDATE alerts SET severity = 'critical' WHERE alert_type IN ('device_unreachable', 'agent_offline')")
