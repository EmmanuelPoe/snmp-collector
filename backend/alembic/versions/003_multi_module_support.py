...
def upgrade() -> None:
    ...
    op.execute("UPDATE devices SET snmp_modules = jsonb_build_array(snmp_module) WHERE snmp_module IS NOT NULL")
    op.execute("UPDATE devices SET snmp_modules = '[\"if_mib\"]'::jsonb WHERE snmp_modules IS NULL")
