...
def test_enqueue_and_fetch_once(client, auth_headers):
    r = client.post(
        "/agents/agent-1/commands", json={"type": "walk", "params": {"base_oid": "1.3.6.1.2.1"}}, headers=auth_headers
    )
...
def test_result_and_get(client, auth_headers):
    cid = client.post("/agents/a/commands", json={"type": "walk", "params": {}}, headers=auth_headers).json()[
        "command_id"
    ]
...
def test_result_unknown_command_404(client, auth_headers):
    assert client.post("/commands/nope/result", json={"status": "done"}, headers=auth_headers).status_code == 404
