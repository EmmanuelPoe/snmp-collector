...
def two_agents(client, auth_headers, mock_backend_empty):
    ...
    for host in ("host-a", "host-b"):
        resp = client.post("/register", json={"hostname": host, "ip": "10.0.0.1"}, headers=auth_headers)
...
def test_command_result_only_from_target_agent(client, two_agents, auth_headers):
    ...
    cid = client.post(f"/agents/{agent_a}/commands", json={"type": "walk", "params": {}}, headers=auth_headers).json()[
        "command_id"
    ]
    resp = client.post(f"/commands/{cid}/result", json={"status": "done", "result": []}, headers=cred_b)
    assert resp.status_code == 403
    resp = client.post(f"/commands/{cid}/result", json={"status": "done", "result": []}, headers=cred_a)
    assert resp.status_code == 200
...
def test_enforce_rejects_shared_key_on_agent_routes(client, two_agents, auth_headers, mock_backend_empty, monkeypatch):
    import config
    ...
    assert client.post("/register", json={"hostname": "h2", "ip": "9.9.9.9"}, headers=auth_headers).status_code == 200


def test_ingest_requires_agent_credential_under_enforce(
    client, two_agents, auth_headers, sample_polls_parquet, monkeypatch, reset_db
):
    import config
